...
{ 
	private static final long serialVersionUID = 6319497617420853788L;
    ...
    {
        start = new JButton("开始截屏");
        cancel = new JButton("缩小至托盘");
        
        start.addActionListener(this);
        cancel.addActionListener(this);
        JPanel buttonJP = new JPanel();
        c = new JPanel(new BorderLayout());
        JLabel jl = new JLabel("屏幕截图工具", JLabel.CENTER);
        JLabel jl1 = new JLabel("作者: 谢玉伸", JLabel.CENTER);
    ...
    */
    ...
    private class Temp extends JPanel implements MouseListener, MouseMotionListener
    {
        ...
        {  
            ...
            g.fillRect(Math.min(startX, endX), Math.min(startY, endY), Math.abs(startX - endX), Math.abs(startY - endY));
    ...
    public void setTray()
    {
        if (SystemTray.isSupported())   // 判断是否支持系统托盘
        {
            ...
            open_main = new MenuItem("打开主界面");	//创建菜单项 
            capture =  new MenuItem("截图");	
            exit = new MenuItem("退出");
            
            popupMenu.add(open_main); // 为弹出菜单添加菜单项 
    ...
    {
        //打开主界面菜单项事件监听
        open_main.addActionListener(new ActionListener()
        {
            @Override
            public void actionPerformed(ActionEvent e)
            {
